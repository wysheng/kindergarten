	...
	$.post('/kindergarten/stuSickCheck/getStuMChkDetailList?ttid=' + Math.random(), {pageNo:pageNo, pageSize:pageSize, classes:classes,isTakePills:isTakePills}, function(json, textStatus){
		...
		try {
			...
			$.each(stuMChkDetailList, function(index, data){
				...
				str += '<td width="10%" align="left" class=sec1>'+data.sickLevel+'</a></td>';
				...
				if(acqx != 1){
					str += '<td width="20%" align="center" class=sec1><a href="javascript:updateStuMChk('+data.id+')">修改</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:delStuMchk('+data.id+')">删除</a></td>';
				}
				str += '</tr>';
			});
...
//修改生病记录
function updateStuMChk(id){
	window.location.href = '/kindergarten/stuSickCheck/getStuMchkById/' + id + '-updateStuMchk';
}
//查看公告内容
