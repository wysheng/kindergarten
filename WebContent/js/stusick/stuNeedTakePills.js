	...
	$.post('/kindergarten/stuSickCheck/getStuSickCheckList?ttid=' + Math.random(), {pageNo:pageNo, pageSize:pageSize, title:title,isTakePills:isTakePills}, function(json, textStatus){
		...
		try {
			...
			$.each(stuMChkList, function(index, data){
				...
				if(acqx != 1){
					str += '<td width="20%" align="center" class=sec1><a href="javascript:getStuNeedTakePillsDetailList('+data.classes+')">详情</a></td>';
				}
				str += '</tr>';
			});
...
function getStuNeedTakePillsDetailList(classes){
	window.location.href = '/kindergarten/pages/stusick/stuNeedTakePillsByClasses.jsp?classes='+classes;
