$(document).ready(function(){
	...
	$("#search").click(function(){
		...
		window.location.href="/kindergarten/pages/oldpaike.jsp?pno=1&ntitle="+encodeURIComponent(encodeURIComponent(ntitle));
	});
...
function getPaike(pageNo, pageSize, ntitle){
	var former = '/kindergarten/pages/oldpaike';
	var latter = '.jsp';
	var acid = $("#acid").val();
	var str = '';
	$.post('/kindergarten/paike/getOldPaikeList?ttid=' + Math.random(), {pageNo:pageNo, pageSize:pageSize, acid:acid, title:ntitle}, function(json, textStatus){
		if(json == null)
			return;
		try {
			...
			$.each(paikeList, function(index, data){
				...
				str += '<td width="30%" align="center" class=sec1><a href="javascript:lookpaike('+data.id+')">'+data.title+'</a></td>';
