$(document).ready(function(){
	...
	$("#search").click(function(){
		...
		if(ntitle == null || ntitle == "" || ntitle == "输入名称关键字查询")
			ntitle = '';
		window.location.href = '/kindergarten/pages/oldkecheng.jsp?pno=1&ntitle=' + encodeURIComponent(encodeURIComponent(ntitle));
	});
...
function getKecheng(pageNo, pageSize, ntitle){
	var former = '/kindergarten/pages/oldkecheng';
	var latter = '.jsp';
	var acid = $("#acid").val();
	var str = '';
	$.post('/kindergarten/kecheng/getOldKechengList?ttid=' + Math.random(), {pageNo:pageNo, pageSize:pageSize, acid:acid, title:ntitle}, function(json, textStatus){
		if(json == null)
			return;
		try {
			var list = json.list;
			...
			$.each(list, function(index, data){
				...
				str += '<td width="9%" align="center" class=sec1>'+data.kcnj+'</td>';
				str += '<td width="9%" align="center" class=sec1>'+data.kcnf+'</td>';
				/*str += '<td width="9%" align="center" class=sec1>'+data.kczy+'</td>';
				str += '<td width="9%" align="center" class=sec1>'+data.kcxf+'</td>';*/
